...
export class MetashrewOrd {
  ...
  constructor({ baseUrl, programHash, blockTag }: any) {
    ...
    this.programHash =
      ...
      ethers.solidityPackedKeccak256(
        ["bytes"],
        [ethers.hexlify(fs.readFileSync(process.env.PROGRAM_PATH))],
      );
    this.blockTag = blockTag;
  }
  async _call({ method, input }): Promise<string> {
  ...
  async satranges({ outpoint }: any): Promise<any> {
    const [hash, vout] = outpoint.split(":");
    const buffer = ethers.hexlify(
      ordinals.SatRangesRequest.toBinary({
        outpoint: {
    ...
    return ordinals.SatRangesResponse.fromBinary(
      ethers.toBeArray(byteString),
    ).satranges.ranges.map((v) => ({
      start: v.start,
      distance: v.distance,
    }));
  }
